...
if [ -d /usr/local/jamf/bin/jamfAAD ]; then 
    zip -r $0DCFILENAME /usr/local/jamf/bin/jamfAAD/*
else
...
echo -e "*************************\n"      > ./pkgutil_info.txt
echo -e "pkgutil-info <package name\n\n"  >> ./pkgutil_info.txt
pkgutil --pkgs | grep -v com.apple.pkg.MAContent10 | sort | while read x; do (pkgutil --pkg-info $x; eco -e ""); done >> ./pkgutil_info.txt

    zip -r $0DCFILENAME •/pkgutil_pkgs.txt
    zip -r $0DCFILENAME •/pkgutil_info.txt

    echo "Gathering syslogs. This may take a few minutes, got yourself a drink"
...
if [ -d /usr/local/jamf/bin/jamfAAD ]; then
    log show --style syslog --info --debug --predicate 'subsystem CONTAINS "jamfAAD"' --last 30d >> ./syslog_jamfAAD.log
        ...
        echo -e "/user/local/jamf/bin/jamfAAD not found, skipping JAMF" >> ./syslog_jamfAAD.log
fi 
...
profiles list -all -verbose >> ./IntuneProfiles.txt

echo -e "profiles show\n*************************\n\n" > ./IntuneProfiles.txt
profiles show >> ./IntuneProfiles.txt

echo -e "profiles show -verbose -all\n*************************\n\n" > ./IntuneProfiles.txt
